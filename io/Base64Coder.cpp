...
namespace io
{
    const char* Base64Coder::IllegalLengthException::what() const noexcept
    {
        return "$Length_is_not_a_multiple_of_4.";
    }

    const char* Base64Coder::IllegalCharacterException::what() const noexcept
    {
    ...
    std::string Base64Coder::decode(const std::string& in)
    {
        ...
        char out[oLen + 1];  // +1 to make llvm static analyzer happy
        size_t op = 0;
        for (size_t ip = 0; ip < iLen;) {
            int i0 = in[ip++];
            int i1 = in[ip++];
