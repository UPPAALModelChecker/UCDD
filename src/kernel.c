 ...
 */
#define bddHash(f, g) ((((uintptr_t)(f)*DD_P1 + (uintptr_t)(g)) * DD_P2))
...
int32_t cdd_is_extra_terminal(ddNode* node) { return cdd_rglr(node) != cddfalse && cdd_isterminal(node); }
...
int32_t cdd_eval_true(ddNode* node) { return ((node) == cddtrue || (cdd_is_extra_terminal(node) && !cdd_mask(node))); }

int32_t cdd_eval_false(ddNode* node) { return ((node) == cddfalse || (cdd_is_extra_terminal(node) && cdd_mask(node))); }
...
void cdd_default_rehashhandler(CddRehashStat* s)
{
    fprintf(stderr, "Rehash #%d: level %d / %d buckets / %d keys / %d max / %.1fs / %.1fs total\n", s->num, s->level,
            s->buckets, s->keys, s->max, ((double)s->time) / CLOCKS_PER_SEC, ((double)s->sumtime) / CLOCKS_PER_SEC);
}
...
ddNode* cdd_interval(int32_t i, int32_t j, raw_t low, raw_t high)
{
    return (i > j) ? cdd_interval_from_level(cdd_diff2level[cdd_difference(i, j)], low, high)
                   : cdd_interval_from_level(cdd_diff2level[cdd_difference(j, i)], bnd_u2l(high), bnd_l2u(low));
}
...
void cdd_markcount(ddNode* node, int32_t* cnt)
{
    ...
    case TYPE_CDD:
        ...
        break;
    case TYPE_BDD: cdd_markcount(bdd_node(node)->low, cnt); cdd_markcount(bdd_node(node)->high, cnt);
    }
}
...
void cdd_add_clocks(int32_t n)
{
    ...
    add_levels(diffs);
    cdd_diff2level = realloc(cdd_diff2level, cdd_difference_count(cdd_clocknum + n) * sizeof(LevelInfo*));
