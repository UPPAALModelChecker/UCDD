...
void cdd_tarjan_init(struct tarjan* graph, uint32_t dim, struct distance* dist, uint32_t* count, struct edge* edges,
                     struct node* fifo, uint32_t* queued)
{
    assert(dim > 0);
    graph->dim = dim;
    graph->count = count;
 ...
 */
static int disassemble(uint32_t root, uint32_t node, uint32_t terminal, struct node* preorder, uint32_t* depth,
                       struct node* queue, uint32_t* queued)
{
    assert(root != node);
